			...
			<span
				...
				onClick={e => {
					if (!tabTileProps.showDash) {
						...
						setSelectedTabIdForDM(parseInt(tabId));
						setSelectedTileIdForDM(parseInt(tileId));
						setSelectedDynamicMeasureId(parseInt(dmId));
						setSelectedToEdit(parseInt(tabId), parseInt(tileId), parseInt(dmId), true);
			...
			>
				{!tabTileProps.showDash ? (
					<div style={{ display: "block", width: "100%" }}>
						...
						<Button
							style={{ float: "right", marginTop: "-50px", height: "30px" }}
							sx={{
								textTransform: "none",
...
const mapDispatchToProps = dispatch => {
	return {
		...
		clearRichText: propKey => dispatch(clearRichText(propKey)),
		setDynamicMeasureWindowOpen: (propKey, chartValue) =>
			dispatch(setDynamicMeasureWindowOpen(propKey, chartValue)),
		addNewDynamicMeasurePropsFromNewTab: (tabId, tileId, dynamicMeasureId, dataset) =>
		...
		setSelectedTileIdForDM: tileId => dispatch(setSelectedTileIdInDynamicMeasureState(tileId)),
		setSelectedTabIdForDM: tabId => dispatch(setSelectedTabIdInDynamicMeasureState(tabId)),
